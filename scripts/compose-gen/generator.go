...
func writeFilterCurrency(b *strings.Builder, cfg *Config) {
	...
	for i := range cfg.FilterCurrency {
		...
		fmt.Fprintf(b, "      - INPUT_QUEUE=Q1234_transfers_queue_%d\n", i)
		fmt.Fprintf(b, "      - INPUT_ROUTING_KEYS=Q1234_filter_currency_shard-%d\n", i)
...
func writeFilterAmount(b *strings.Builder, cfg *Config) {
	for i := range cfg.FilterAmount {
		...
		fmt.Fprintf(b, "      - INPUT_QUEUE=Q1_filtered_%d\n", i)
		fmt.Fprintf(b, "      - INPUT_ROUTING_KEYS=Q1_filtered_shard-%d\n", i)
		b.WriteString("      - OUTPUT_EXCHANGE=results_exchange\n")
...
func writeFilterAndSplitterQ4(b *strings.Builder, cfg *Config) {
	for i := range cfg.FilterAndSplitterQ4 {
		...
		b.WriteString("      - INPUT_EXCHANGE=Q1234_filtered_exchange\n")
		fmt.Fprintf(b, "      - INPUT_QUEUE=Q4_filter_range_in_%d\n", i)
...
func writeFilterDateAndPayment(b *strings.Builder, cfg *Config) {
	for i := range cfg.FilterDateAndPayment {
		...
		fmt.Fprintf(b, "      - ID=%d\n", i)
		b.WriteString("      - INPUT_EXCHANGE=transfers_exchange\n")
		fmt.Fprintf(b, "      - INPUT_ROUTING_KEYS=Q5_filter_shard-%d\n", i)
		fmt.Fprintf(b, "      - INPUT_QUEUE=Q5_transfers_queue_%d\n", i)
