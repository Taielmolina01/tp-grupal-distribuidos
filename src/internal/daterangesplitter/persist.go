...
func unmarshalClientState(data []byte) (*clientState, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("daterangesplitter: unmarshal tracker: %w", err)
	}

	outTracker, err := outputtracker.Unmarshal(r)
	if err != nil {
		return nil, fmt.Errorf("daterangesplitter: unmarshal outputTracker: %w", err)
	}

	if r.Err() != nil {
		return nil, fmt.Errorf("daterangesplitter: unmarshal: %w", r.Err())
	}
